...
// Adding eventListener to the search bar
...
async function searchHeros(textSearched) {
     ...
     await fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${textSearched}&apikey=9ab871748d83ae2eb5527ffd69e034de&hash=d35377547e551cd64a60657d2517bb7f?ts=1`)
          .then(res => res.json()) //Converting the result data into JSON format
...
// An array of search results is accepted as argument 
...
function showSearchedResults(searchedHero) {
     ...
     // IDs of the character which are added in the favourites Used 
     //for displaying the appropriate button in search results i.e
     ...
     searchResults.innerHTML = ``;
     // count variable is used to count the result displayed in DOM
     let count = 1;
...
(function (){
     ...
     switch(currentTheme){
          ...
          case "dark":
               ...
               themeButton.childNodes[0].style.color = "yellow";
               break;
     }
})();
